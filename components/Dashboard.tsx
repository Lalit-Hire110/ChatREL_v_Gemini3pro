...
import { Heart, Activity, BrainCircuit, TrendingUp, Hash } from 'lucide-react';
      ...
      <div className="lg:col-span-1 bg-white rounded-2xl shadow-lg border border-slate-100 p-6">
        ...
        <div className="flex flex-wrap gap-2 justify-center content-start h-[250px] overflow-y-auto scrollbar-hide">
            {result.wordCloud.map((item, i) => (
                <span 
                    key={i} 
                    className="px-3 py-1 rounded-full bg-slate-50 text-slate-600 transition-all hover:bg-blue-50 hover:text-blue-600 hover:scale-105 cursor-default select-none flex items-center"
                    style={{ 
                ...
                <ul className="space-y-3">
                    {result.keyInsights.map((insight, i) => (
                        <li key={i} className="flex items-start gap-3 text-sm text-slate-600">
                            <span className="mt-1 w-5 h-5 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center text-xs font-bold flex-shrink-0">
                                {i + 1}
                            </span>
                            {insight}
                        </li>
